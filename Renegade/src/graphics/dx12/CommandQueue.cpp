	...
	namespace graphics
	{
		namespace dx12
		{
				...
				m_d3d12Device(device)
			{
				...
				{
					LOG(LOGSEVERITY_ERROR, "Failed creating command queue.");
					return;
				}
			...
			Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList2> CommandQueue::GetCommandList()
			{
				...
				if (!m_CommandAllocatorQueue.empty() && IsFenceComplete(m_CommandAllocatorQueue.front().fenceValue))
				{
					...
					{
						LOG(LOGSEVERITY_ERROR, "Failed resetting command allocator.");
						return nullptr;
					}
				}
				else
				...
				if (!m_CommandListQueue.empty())
				{
					...
					{
						LOG(LOGSEVERITY_ERROR, "Failed resetting command list.");
						return nullptr;
					}
				}
				else
				...
				{
					LOG(LOGSEVERITY_ERROR, "Failed associating command allocator with command list.");
					return nullptr;
				}
			...
			uint64_t CommandQueue::ExecuteCommandList(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList2> commandList)
			{
				...
				{
					LOG(LOGSEVERITY_ERROR, "Failed setting private data for command list.");
					return -1;
				}
