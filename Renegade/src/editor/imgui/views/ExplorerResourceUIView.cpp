	...
	namespace editor
	{
		namespace imgui
		{
			...
			void CreateExplorerResourceUIViewChild(ExplorerResourceUIView& a_ResourceView, ExplorerResource& a_Resource, ImGuiWindow& a_Window)
			{
				...
				{
					ExplorerResourceUIView resourceToAdd = ExplorerResourceUIView(a_Window, *resource);
			...
			{
				ImGui::PopGuard popGuard;

				ImVec2 toolbarSize = ImVec2(ImGui::GetContentRegionAvail().x, core::ENGINE.GetEditor().GetImGuiWindow().HeaderSize().y * 2.5f);
				ImGui::BeginToolbar(toolbarSize);

				popGuard.PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0, 0));
				popGuard.PushStyleVar(ImGuiStyleVar_FrameRounding, 0);
				...
				if (ImGui::TransparentButton(IMGUI_FORMAT_ID(std::string(ICON_FA_ARROW_TURN_UP), BUTTON_ID, "SHOW_IN_EXPLORER_INSPECTOR").c_str(), ImVec2(0, core::ENGINE.GetEditor().GetImGuiWindow().HeaderSize().y)))
				{
					file::FileLoader::OpenInExplorer(m_Resource.GetParent()->GetPath().c_str());
				}
				ImGui::SameLine();
				if (ImGui::TransparentButton(IMGUI_FORMAT_ID(std::string(ICON_FA_DELETE), BUTTON_ID, "DELETE_INSPECTOR").c_str(), ImVec2(0, core::ENGINE.GetEditor().GetImGuiWindow().HeaderSize().y)))
				{
					...
					m_ResourceData.scene = nullptr;
					m_ResourceData.image = nullptr;
					return;
				}
